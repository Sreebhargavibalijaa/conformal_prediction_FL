...
from fcp_pruning import get_indices
import time
from statistics import mean
from torch.quantization import quantize, prepare, convert
from epirical_average import empircal_average
...
# Load the MNIST dataset
        ...
        for epoch in range(5):  # Adjust the number of epochs as needed
            ...
            print(
            f"Client #{idx} | Epoch: {epoch}/{self.args.n_client_epochs} | Loss: {epoch_loss} | Acc: {epoch_acc}",
            end="\r",
        )
    ...
    # Append the quantized model to the list
        ...
        return quantized_client_model.state_dict(),epoch_loss,quantized_client_model,client_datasets
    ...
    def _train_client(
        ...
        q_model_weights_each_client, epoch_loss, model,client_datasets = self.quantize_models_final(model,client_idx)
        # quantized_model = self.quantized_model_accuracies(model, logits, target)
        return model, epoch_loss / self.args.n_client_epochs, q_model_weights_each_client,client_datasets
    def pruned_weights(self, weights, get_weights_indces):        
        final_weights = []
        for j in range(0,len(get_weights_indces)):
            final_weights.append(weights[get_weights_indces[j]])
        final_global_weihgts = [sum(values) for values in zip(*final_weights)]
        final_global_weights = [i/len(get_weights_indces) for i in final_global_weihgts]
        return final_global_weights
    def train(self) -> None:
        ...
        for epoch in range(self.args.n_epochs):
            ...
            idx_clients = np.random.choice(range(self.args.n_clients), self.args.n_clients , replace=False)
            # Train clients
            ...
            for client_idx in idx_clients:
                ...
                # Train client
                client_model, client_loss, quantized_model_weights_each_client,client_datasets = self._train_client(
                    root_model=self.root_model,
                    train_loader=self.train_loader,
                    client_idx=client_idx, 
                )
                quantized_models_weights.append(list(quantized_model_weights_each_client.values()))
                # finalls = sum(list(quantized_model_weights_each_client.values()))
                # weights = weights + list(quantized_model_weights_each_client.values())
            ...
            get_weights_indces = get_indices(client_datasets,self.args.n_clients,self.args.n_client_epochs,clients_models,self.args.prune_percent,self.args.model_name)
            weights = self.pruned_weights(quantized_models_weights, get_weights_indces)
# Load weights into the model
            ...
            if (epoch + 1) % self.args.log_every == 0:
                ...
                each_round_accurcaies.append(total_acc)
                print(
        ...
        print("emmpirical average")
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))])
        print(empircal_average(self.args.model_name,self.root_model,datasets.FashionMNIST(root='./data', train=True, download=True, transform=transform),self.args.n_clients))
        print(each_round_accurcaies)
        sn = []
        for r in range(0, self.args.n_epochs):
            sn.append(r)
        x = np.array(sn)
        y = np.array(each_round_accurcaies)
        plt.plot(x, y)
