...
class FedAvg:
    ...
    def _get_data(
        ...
        """
        train_set = datasets.ImageNet(root=root, train=True)
        test_set = datasets.ImageNet(root=root, train=False)
...
# Load the MNIST dataset
        mnist_dataset = datasets.ImageNet(root='./data', train=True, download=True, transform=transform)
    ...
    def _train_client(
        ...
        q_model_weights_each_client = self.quantize_models_final(model,client_idx)
        # quantized_model = self.quantized_model_accuracies(model, logits, target)
        return model, epoch_loss / self.args.n_client_epochs, q_model_weights_each_client
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.args.n_epochs):
            ...
            for client_idx in idx_clients:
                ...
                # Train client
                client_model, client_loss, quantized_model_weights_each_client = self._train_client(
                    root_model=self.root_model,
                    train_loader=self.train_loader,
                    client_idx=client_idx,
                )
                quantized_models_weights.append(quantized_model_weights_each_client)
            ...
            weights_test = [a/len(weights) for a in weights]# sum(list(quantized_models_weights))/len(quantized_models_weights)
            from statistics import mean
            # avg_updated_weights = mean(quantized_models_weights)
            # updated_weights = fedmed(clients_models)

# Load weights into the model
            self.root_model.load_state_dict(dict(zip(self.root_model.state_dict().keys(),weights_test)))
