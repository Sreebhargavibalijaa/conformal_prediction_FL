...
from test_final import get_indices
...
# Load the MNIST dataset
    ...
    def pruned_weights_30(self, weights, get_weights_indces):        
        final_weights = []
        for i in range(1,14):
            final_weights= final_weights + (weights[get_weights_indces[i]])
        return final_weights
    def pruned_weights_50(self, weights, get_weights_indces):        
        final_weights = []
        for i in range(1,10):
            final_weights= final_weights + (weights[get_weights_indces[i]])
        return final_weights
    def pruned_weights_70(self, weights, get_weights_indces):        
        final_weights = []
        for i in range(1,6):
            final_weights= final_weights + (weights[get_weights_indces[i]])
        return final_weights
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.args.n_epochs):
            ...
            idx_clients = np.random.choice(range(self.args.n_clients), 2 , replace=False)
            # Train clients
            ...
            # weights_test = [a/len(weights) for a in weights]
            # psets_scores = []
            get_weights_indces = get_indices(client_datasets,self.args.n_clients,self.args.n_client_epochs,clients_models)
            weights = self.pruned_weights_30(quantized_models_weights, get_weights_indces)
            # weights = sum(list(final_weights))/len(final_weights)
